...
!
DO JI=1,KSIZE 
  GNEGT(JI)=PT(JI)<CST%XTT .AND. PRVT(JI)>ICED%XRTMIN(1) .AND. ODCOMPUTE(JI)
ENDDO
...
ZZW(:)=0.
DO JI=1,KSIZE 
  IF(GNEGT(JI) .AND. PT(JI)<CST%XTT-5.0 .AND. ZSSI(JI)>0.0 )THEN
    ZZW(JI)=ICEP%XNU20*EXP(ICEP%XALPHA2*ZSSI(JI)-ICEP%XBETA2)
  ELSEIF(GNEGT(JI) .AND. PT(JI)<=CST%XTT-2.0 .AND. PT(JI)>=CST%XTT-5.0 .AND. ZSSI(JI)>0.0)THEN
