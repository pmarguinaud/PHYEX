...
IF(.NOT. LDSOFT) THEN
  ...
  IF(IGDRY>0) THEN
    ...
    DO JL=1,KSIZE 
    IF(GDRY(JL))THEN
      PRG_TEND(JL,IRRDRYG) = ICEP%XFRDRYG*ZZW(JL)                    & ! RRDRYG
                        *( PLBDAR(JL)**(-4) )*( PLBDAG(JL)**ICED%XCXG ) &
                               *( PRHODREF(JL)**(-ICED%XCEXVT-1.) )   &
                    *( ICEP%XLBRDRYG1/( PLBDAG(JL)**2              ) + &
                       ICEP%XLBRDRYG2/( PLBDAG(JL)   * PLBDAR(JL)   ) + &
                       ICEP%XLBRDRYG3/(               PLBDAR(JL)**2) )
    ENDIF
    ENDDO
  ENDIF
ENDIF
...
IF(KRR==7) THEN
  ...
  IF(LDWETG(JL))THEN
    ...
    PRWETGH(JL)=(MAX(0., PRGSI(JL)+PRICFRRG(JL)+PRRCFRIG(JL))+ZRWETG_INIT(JL))*&
                    &ZRDRYG_INIT(JL)/(ZRWETG_INIT(JL)+ZRDRYG_INIT(JL))
    PRWETGH_MR(JL)=MAX(0., PRGSI_MR(JL))*ZRDRYG_INIT(JL)/(ZRWETG_INIT(JL)+ZRDRYG_INIT(JL))
  ELSE
    PRWETGH(JL)=0.
    PRWETGH_MR(JL)=0.
  ENDIF
