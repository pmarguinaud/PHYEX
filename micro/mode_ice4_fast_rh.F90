...
IF(.NOT. LDSOFT) THEN
  IF(IGWET>0)THEN
    ...
    DO JL=1,KSIZE 
    IF(GWET(JL))THEN
      PRH_TEND(JL,IRGWETH)=ICEP%XFGWETH*ZZW(JL)                       & ! RGWETH
                    *( PLBDAG(JL)**(ICED%XCXG-ICED%XBG) )*( PLBDAH(JL)**ICED%XCXH )  &
                       *( PRHODREF(JL)**(-ICED%XCEXVT-1.) )               &
                       *( ICEP%XLBGWETH1/( PLBDAH(JL)**2              ) + &
                          ICEP%XLBGWETH2/( PLBDAH(JL)   * PLBDAG(JL)   ) + &
                          ICEP%XLBGWETH3/(               PLBDAG(JL)**2) )
      PRH_TEND(JL,IRGDRYH)=PRH_TEND(JL,IRGWETH)
    ENDIF
    !When graupel grows in wet mode, graupel is wet (!) and collection efficiency must remain the same
    IF(GWET(JL) .AND. .NOT. LDWETG(JL))THEN
      PRH_TEND(JL,IRGDRYH)=PRH_TEND(JL,IRGDRYH)*(ICEP%XCOLGH*EXP(ICEP%XCOLEXGH*(PT(JL)-CST%XTT)))
    ENDIF
    ENDDO
  END IF
ENDIF
...
IF(.NOT. LDSOFT) THEN
  IF(IGWET>0)THEN
    ...
    DO JL=1,KSIZE 
    IF(GWET(JL))THEN
      PRH_TEND(JL,IRRWETH) = ICEP%XFRWETH*ZZW(JL)                    & ! RRWETH
                        *( PLBDAR(JL)**(-4) )*( PLBDAH(JL)**ICED%XCXH ) &
                               *( PRHODREF(JL)**(-ICED%XCEXVT-1.) )   &
                    *( ICEP%XLBRWETH1/( PLBDAH(JL)**2              ) + &
                       ICEP%XLBRWETH2/( PLBDAH(JL)   * PLBDAR(JL)   ) + &
                       ICEP%XLBRWETH3/(               PLBDAR(JL)**2) )
    ENDIF
    ENDDO
  ENDIF
ENDIF
...
IF(PARAMI%LCONVHG)THEN
  DO JL=1,KSIZE 
  IF(LLDRYH(JL))THEN
    ZRDRYHG(JL)=ZRDRYH_INIT(JL)*ZRWETH_INIT(JL)/(ZRDRYH_INIT(JL)+ZRWETH_INIT(JL))
  ELSE
    ZRDRYHG(JL)=0.
  ENDIF
  ENDDO
