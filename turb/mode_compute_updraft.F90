...
DO JI=D%NIB,D%NIE 
  PTHL_UP(JI,D%NKB)= ZTHLM_F(JI,D%NKB)+MAX(0.,MIN(ZTMAX,(PSFTH(JI)/SQRT(ZTKEM_F(JI,D%NKB)))* &
  &PARAMMF%XALP_PERT))
  PRT_UP(JI,D%NKB) = ZRTM_F(JI,D%NKB)+MAX(0.,MIN(ZRMAX,(PSFRV(JI)/SQRT(ZTKEM_F(JI,D%NKB)))* &
  &PARAMMF%XALP_PERT)) 
ENDDO
...
DO JK=D%NKB,D%NKE-D%NKL,D%NKL
  ...
  IF (OENTR_DETR) THEN
    ...
    !       Computation of updraft characteristics at level JK+KKL
      IF(GTEST(JI))THEN
        ZMIX1(JI)=0.5*(PZZ(JI,JK+D%NKL)-PZZ(JI,JK))*(PENTR(JI,JK)-PDETR(JI,JK))
  ...
  IF (OENTR_DETR) THEN
    ...
    ! compute frac_up at JK+KKL
      IF (GTEST(JI))THEN
        PFRAC_UP(JI,JK+D%NKL)=PEMF(JI,JK+D%NKL)/(SQRT(ZW_UP2(JI,JK+D%NKL))*ZRHO_F(JI,JK+D%NKL))
      ENDIF
...
!  Compute cons then non cons. var. of mixture at the flux level KK+KKL  with initial ZKIC
  ZMIXTHL(JI) = ZKIC_INIT * 0.5*(PTHLM(JI,KK)+PTHLM(JI,KK+KKL))+(1. - ZKIC_INIT)*PTHL_UP(JI)
  ZMIXRT(JI)  = ZKIC_INIT * 0.5*(PRTM(JI,KK)+PRTM(JI,KK+KKL))+(1. - ZKIC_INIT)*PRT_UP(JI)
ENDDO
CALL TH_R_FROM_THL_RT(CST,NEB,D%NIT,HFRAC_ICE,ZFRAC_ICE,&
