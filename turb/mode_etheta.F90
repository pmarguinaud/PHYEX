 ...
 ELSE IF ( KRR == 1 ) THEN                           ! only vapor
  ZDELTA = (CST%XRV/CST%XRD) - 1.
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PETHETA(JI,JJ,JK) = 1. + ZDELTA*PRM(JI,JJ,JK,1)
        ENDDO
      ENDDO
    ENDDO
...
!
  ...
  !
            PETHETA(JI,JJ,JK) = ZA(JI,JJ,JK)                                                 &
            +( PLOCPEXNM(JI,JJ,JK) * ZA(JI,JJ,JK) -(1.+ZDELTA) * (PTHLM(JI,JJ,JK) &
            + PLOCPEXNM(JI,JJ,JK)*PRM(JI,JJ,JK,2))   &
            / (1. + ZRW(JI,JJ,JK))                                 &
            ) * PATHETA(JI,JJ,JK) * 2. * PSRCM(JI,JJ,JK)
          ENDDO
        ENDDO
