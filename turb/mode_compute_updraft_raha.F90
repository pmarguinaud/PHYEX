...
DO JK=D%NKB,D%NKE-D%NKL,D%NKL   !  Vertical loop
  DO JI=D%NIB,D%NIE 
    ...
    IF ((ZTHVM_F(JI,JK+D%NKL)<ZTHVM_F(JI,JK)) .AND. (ZTHVM_F(JI,D%NKB)>=ZTHVM_F(JI,JK)))THEN
...
DO JK=D%NKB,D%NKE-D%NKL,D%NKL
  ...
  ! If the updraft did not stop, compute cons updraft characteritics at jk+1
    IF(GTEST(JI))     THEN
      ...
      ZTHS_UP(JI,JK+D%NKL)=(ZTHS_UP(JI,JK)*(1.-0.5*ZMIX2(JI)) + ZTHSM(JI,JK)*ZMIX2(JI))&
      /(1.+0.5*ZMIX2(JI))   
...
! Updraft fraction must be smaller than XFRAC_UP_MAX
      ...
      PEMF(JI,JK+D%NKL) = ZRHO_F(JI,JK+D%NKL)*PFRAC_UP(JI,JK+D%NKL)*SQRT(ZW_UP2(JI,JK+D%NKL))
    ENDIF
  ENDDO
ENDDO
