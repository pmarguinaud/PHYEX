...
DO JK=1,D%NKT 
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK) = ZMZM_RHODJ(JI,JJ,JK)*PDFDDTDZ(JI,JJ,JK)/PDZZ(JI,JJ,JK)**2
  ENDDO
 ENDDO
ENDDO
!
...
ZY(JI,JJ,IKB) = PRHODJ(JI,JJ,IKB)*PVARM(JI,JJ,IKB)/PTSTEP                  &
    - ZMZM_RHODJ(JI,JJ,IKB+D%NKL) * PF(JI,JJ,IKB+D%NKL)/PDZZ(JI,JJ,IKB+D%NKL)    &
    + ZMZM_RHODJ(JI,JJ,IKB) * PF(JI,JJ,IKB)/PDZZ(JI,JJ,IKB)          &
    + ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,IKB+D%NKL) * PIMPL * PVARM(JI,JJ,IKB+D%NKL) &
    - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,IKB+D%NKL) * PIMPL * PVARM(JI,JJ,IKB)
 ENDDO
ENDDO
!
DO JJ=1,D%NJT 
...
DO JK=IKTB+1,IKTE-1
  ZY(JI,JJ,JK) = PRHODJ(JI,JJ,JK)*PVARM(JI,JJ,JK)/PTSTEP                 &
    - ZMZM_RHODJ(JI,JJ,JK+D%NKL) * PF(JI,JJ,JK+D%NKL)/PDZZ(JI,JJ,JK+D%NKL)     &
    + ZMZM_RHODJ(JI,JJ,JK) * PF(JI,JJ,JK)/PDZZ(JI,JJ,JK)           &
    + ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK+D%NKL) * PIMPL * PVARM(JI,JJ,JK+D%NKL) &
    - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK+D%NKL) * PIMPL * PVARM(JI,JJ,JK)   &
    - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK) * PIMPL * PVARM(JI,JJ,JK)   &
    + ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK) * PIMPL * PVARM(JI,JJ,JK-D%NKL)
END DO
 ENDDO
ENDDO
! 
...
ZY(JI,JJ,IKE) = PRHODJ(JI,JJ,IKE)*PVARM(JI,JJ,IKE)/PTSTEP               &
    - ZMZM_RHODJ(JI,JJ,IKE+D%NKL) * PF(JI,JJ,IKE+D%NKL)/PDZZ(JI,JJ,IKE+D%NKL) &
    + ZMZM_RHODJ(JI,JJ,IKE) * PF(JI,JJ,IKE)/PDZZ(JI,JJ,IKE)       &
    - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,IKE) * PIMPL * PVARM(JI,JJ,IKE)   &
    + ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,IKE) * PIMPL * PVARM(JI,JJ,IKE-D%NKL)
 ENDDO
ENDDO
!
!
...
!
 ...
 DO JI=1,D%NIT 
  ZB(JI,JJ,IKB) =   PRHODJ(JI,JJ,IKB)/PTSTEP                   &
                - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,IKB+D%NKL) * PIMPL
  ZC(JI,JJ,IKB) =   ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,IKB+D%NKL) * PIMPL
!
  DO JK=IKTB+1,IKTE-1
    ZA(JI,JJ,JK) =   ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK) * PIMPL
    ZB(JI,JJ,JK) =   PRHODJ(JI,JJ,JK)/PTSTEP                        &
                            - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK+D%NKL) * PIMPL &
                            - ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK) * PIMPL
    ZC(JI,JJ,JK) =   ZRHODJ_DFDDTDZ_O_DZ2(JI,JJ,JK+D%NKL) * PIMPL
  END DO
!
...
!
  ...
  DO JK = IKB+D%NKL,IKE-D%NKL,D%NKL
    ZGAM(JI,JJ,JK) = ZC(JI,JJ,JK-D%NKL) / ZBET(JI,JJ)  
                                                    ! gam(k) = c(k-1) / bet
    ZBET(JI,JJ)    = ZB(JI,JJ,JK) - ZA(JI,JJ,JK) * ZGAM(JI,JJ,JK)
                                                    ! bet = b(k) - a(k)* gam(k)  
    PVARP(JI,JJ,JK)= ( ZY(JI,JJ,JK) - ZA(JI,JJ,JK) * PVARP(JI,JJ,JK-D%NKL) ) / ZBET(JI,JJ)
                                        ! res(k) = (y(k) -a(k)*res(k-1))/ bet 
  END DO 
  ! special treatment for the last level
  ZGAM(JI,JJ,IKE) = ZC(JI,JJ,IKE-D%NKL) / ZBET(JI,JJ) 
                                                    ! gam(k) = c(k-1) / bet
  ZBET(JI,JJ)     = ZB(JI,JJ,IKE) - ZA(JI,JJ,IKE) * ZGAM(JI,JJ,IKE)
                                                    ! bet = b(k) - a(k)* gam(k)  
  PVARP(JI,JJ,IKE)= ( ZY(JI,JJ,IKE) - ZA(JI,JJ,IKE) * PVARP(JI,JJ,IKE-D%NKL) ) / ZBET(JI,JJ)
                                       ! res(k) = (y(k) -a(k)*res(k-1))/ bet 
!
!*       3.3 going down
!            ----------
