...
!
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
  PEMOIST(JI,JJ,JK) = ZDELTA*PTHLM(JI,JJ,JK)
    ENDDO
 ENDDO
ENDDO
...
!
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
    ZRW(JI,JJ,JK) = ZRW(JI,JJ,JK) + PRM(JI,JJ,JK,3)
    DO JRR=5,KRR
      ZRW(JI,JJ,JK) = ZRW(JI,JJ,JK) + PRM(JI,JJ,JK,JRR)
    ENDDO
  ...
  !
    PEMOIST(JI,JJ,JK) = ZDELTA * (PTHLM(JI,JJ,JK) + PLOCPEXNM(JI,JJ,JK)*(               &
                                                    PRM(JI,JJ,JK,2)+PRM(JI,JJ,JK,4)))&
                            / (1. + ZRW(JI,JJ,JK))                                &
        +( PLOCPEXNM(JI,JJ,JK) * ZA(JI,JJ,JK)                                        &
               -(1.+ZDELTA) * (PTHLM(JI,JJ,JK) + PLOCPEXNM(JI,JJ,JK)*(               &
                                                    PRM(JI,JJ,JK,2)+PRM(JI,JJ,JK,4)))&
                            / (1. + ZRW(JI,JJ,JK))                                &
         ) * PAMOIST(JI,JJ,JK) * 2. * PSRCM(JI,JJ,JK)
      ENDDO
 ENDDO
ENDDO
  ELSE
 ...
 DO JJ=1,D%NJT 
  ...
  !   Amoist* 2 * SRC is computed at line 6
  !
    PEMOIST(JI,JJ,JK) = ZDELTA * (PTHLM(JI,JJ,JK) + PLOCPEXNM(JI,JJ,JK)*PRM(JI,JJ,JK,2))   &
                            / (1. + ZRW(JI,JJ,JK))                                &
        +( PLOCPEXNM(JI,JJ,JK) * ZA(JI,JJ,JK)                                        &
               -(1.+ZDELTA) * (PTHLM(JI,JJ,JK) + PLOCPEXNM(JI,JJ,JK)*PRM(JI,JJ,JK,2))   &
                            / (1. + ZRW(JI,JJ,JK))                                &
         ) * PAMOIST(JI,JJ,JK) * 2. * PSRCM(JI,JJ,JK)
      ENDDO
 ENDDO
