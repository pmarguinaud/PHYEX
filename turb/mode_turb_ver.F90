...
IF (OOCEAN) THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZBETA(JI,JJ,JK) = CST%XG*CST%XALPHAOC
      ENDDO
    ENDDO
  ENDDO
ELSE
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZBETA(JI,JJ,JK) = CST%XG/PTHVREF(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
END IF
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZSQRT_TKE(JI,JJ,JK) = SQRT(PTKEM(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
!
...
IF (.NOT. OHARAT) THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZD(JI,JJ,JK) = (1.+ZREDTH1(JI,JJ,JK)+ZREDR1(JI,JJ,JK)) * &
        &(1.+0.5*(ZREDTH1(JI,JJ,JK)+ZREDR1(JI,JJ,JK)))
      ENDDO
    ENDDO
  ENDDO
ELSE
