...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZCP(JI,JJ,JK)=CST%XCPD
!
      IF (KRR > 0) ZCP(JI,JJ,JK) = ZCP(JI,JJ,JK) + CST%XCPV * PRT(JI,JJ,JK,1)
    ENDDO
  ENDDO
ENDDO
DO JRR = 2,1+KRRL                          ! loop on the liquid components
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE   
        ZCP(JI,JJ,JK)  = ZCP(JI,JJ,JK) + CST%XCL * PRT(JI,JJ,JK,JRR)
      ENDDO
    ENDDO
  ENDDO
END DO
...
DO JRR = 2+KRRL,1+KRRL+KRRI                ! loop on the solid components   
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZCP(JI,JJ,JK)  = ZCP(JI,JJ,JK)  + CST%XCI * PRT(JI,JJ,JK,JRR)
      ENDDO
    ENDDO
  ENDDO
END DO
...
ELSE
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZEXN(JI,JJ,JK) = (PPABST(JI,JJ,JK)/CST%XP00) ** (CST%XRD/CST%XCPD)
      ENDDO
    ENDDO
  ENDDO
END IF
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZCOEF_DISS(JI,JJ,JK) = 1/(ZCP(JI,JJ,JK) * ZEXN(JI,JJ,JK)) 
    ENDDO
  ENDDO
ENDDO
!
...
!
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZT(JI,JJ,JK) =  PTHLT(JI,JJ,JK) * ZEXN(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
!
      ...
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          IF(PRT(JI,JJ,JK,2)+PRT(JI,JJ,JK,4)>0.0)THEN
            ZFRAC_ICE(JI,JJ,JK) = PRT(JI,JJ,JK,4) / ( PRT(JI,JJ,JK,2) & 
            +PRT(JI,JJ,JK,4) )
          ENDIF
        ENDDO
      ENDDO
...
!
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZLOCPEXNM(JI,JJ,JK) = (1.0-ZFRAC_ICE(JI,JJ,JK))*ZLVOCPEXNM(JI,JJ,JK) &
          +ZFRAC_ICE(JI,JJ,JK) *ZLSOCPEXNM(JI,JJ,JK)
          ZAMOIST(JI,JJ,JK) = (1.0-ZFRAC_ICE(JI,JJ,JK))*ZAMOIST(JI,JJ,JK) &
          +ZFRAC_ICE(JI,JJ,JK) *ZAMOIST_ICE(JI,JJ,JK)
          ZATHETA(JI,JJ,JK) = (1.0-ZFRAC_ICE(JI,JJ,JK))*ZATHETA(JI,JJ,JK) &
          +ZFRAC_ICE(JI,JJ,JK) *ZATHETA_ICE(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
  ELSE
...
IF ( KRRL >= 1 ) THEN
  IF ( KRRI >= 1 ) THEN
    ...
    ! Rnp at t
          ...
          + PRRS(JI,JJ,JK,4)
    ! Theta_l at t
          PTHLT(JI,JJ,JK)  = PTHLT(JI,JJ,JK)  - ZLVOCPEXNM(JI,JJ,JK) &
          * PRT(JI,JJ,JK,2) &
          - ZLSOCPEXNM(JI,JJ,JK) * PRT(JI,JJ,JK,4)
          PRTHLS(JI,JJ,JK) = PRTHLS(JI,JJ,JK) - ZLVOCPEXNM(JI,JJ,JK) &
          * PRRS(JI,JJ,JK,2) &
  ...
  ELSE
    ...
    ! Rnp at t
          ...
          PRRS(JI,JJ,JK,1) = PRRS(JI,JJ,JK,1) + PRRS(JI,JJ,JK,2)
    ! Theta_l at t
          PTHLT(JI,JJ,JK)  = PTHLT(JI,JJ,JK)  - ZLOCPEXNM(JI,JJ,JK) &
          * PRT(JI,JJ,JK,2)
          PRTHLS(JI,JJ,JK) = PRTHLS(JI,JJ,JK) - ZLOCPEXNM(JI,JJ,JK) &
          * PRRS(JI,JJ,JK,2)
        ENDDO
      ENDDO
    ENDDO
...
!           ------------------
  ...
  DO JK=1,D%NKT 
    DO JJ=1,D%NJT 
      DO JI=1,D%NIT 
        ZSHEAR(JI,JJ,JK) = SQRT(ZDUDZ(JI,JJ,JK)*ZDUDZ(JI,JJ,JK) + ZDVDZ(JI,JJ,JK)*ZDVDZ(JI,JJ,JK))
      ENDDO
    ENDDO
  ENDDO
    CALL BL89(D,CST,CSTURB,PZZ,PDZZ,PTHVREF,ZTHLM,KRR,ZRM,PTKET,ZSHEAR,ZLM,OOCEAN,HPROGRAM)
...
!           --------------------------------------------------
  ...
  DO JK=1,D%NKT 
    DO JJ=1,D%NJT 
      DO JI=1,D%NIT 
        ZLM(JI,JJ,JK) = MIN(ZLM(JI,JJ,JK),TURBN%XCADAP*ZLMW(JI,JJ,JK))
      ENDDO
    ENDDO
  ENDDO
!
...
!
  ...
  DO JJ=1,D%NJT 
    DO JI=1,D%NIT 
      DO JK=IKTB,IKTE
        ZLM(JI,JJ,JK) = ( 0.5*(PZZ(JI,JJ,JK)+PZZ(JI,JJ,JK+D%NKL)) - &
        & PZZ(JI,JJ,D%NKA+JPVEXT_TURB*D%NKL) ) * PDIRCOSZW(JI,JJ)
        ZLM(JI,JJ,JK) = ZALPHA  * ZLM(JI,JJ,JK) * ZL0 / ( ZL0 + ZALPHA*ZLM(JI,JJ,JK) )
      END DO
    ENDDO
  ENDDO
!
...
IF (OHARAT) THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZLEPS(JI,JJ,JK)=PLENGTHM(JI,JJ,JK)*(3.75**2.)
      ENDDO
    ENDDO
  ENDDO
ELSE
...
IF (ORMC01) THEN
  DO JJ=1,D%NJT 
    DO JI=1,D%NIT 
      ZUSTAR(JI,JJ)=(PSFU(JI,JJ)**2+PSFV(JI,JJ)**2)**(0.25)
    ENDDO
  ENDDO
  IF (KRR>0) THEN
...
IF (HTURBLEN=='ADAP') THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZLEPS(JI,JJ,JK) = MIN(ZLEPS(JI,JJ,JK),ZLMW(JI,JJ,JK)*TURBN%XCADAP)
      ENDDO
    ENDDO
  ENDDO
END IF
...
DO JJ=IJB,IJE 
  DO JI=IIB,IIE 
    ZCDUEFF(JI,JJ) =-SQRT ( (PSFU(JI,JJ)**2 + PSFV(JI,JJ)**2) /               &
#ifdef REPRO48
    (1.E-60 + ZUSLOPE(JI,JJ)**2 + ZVSLOPE(JI,JJ)**2 ) )
#else
    (CST%XMNH_TINY + ZUSLOPE(JI,JJ)**2 + ZVSLOPE(JI,JJ)**2 ) )
#endif                      
!
!*       4.6 compute the surface tangential fluxes
!
    ZTAU11M(JI,JJ) =2./3.*(  (1.+ (PZZ(JI,JJ,IKB+D%NKL)-PZZ(JI,JJ,IKB))  &
    /(PDZZ(JI,JJ,IKB+D%NKL)+PDZZ(JI,JJ,IKB))  &
    )   *PTKET(JI,JJ,IKB)                   &
    -0.5  *PTKET(JI,JJ,IKB+D%NKL)                 &
    )
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      PTP(JI,JJ,JK) = PTP(JI,JJ,JK) &
      + CST%XG / PTHVREF(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
IF(PRESENT(PTPMF))  THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        PTPMF(JI,JJ,JK)=CST%XG / PTHVREF(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
END IF
...
IF(PRESENT(PDRUS_TURB)) THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        PDRUS_TURB(JI,JJ,JK)   = PRUS(JI,JJ,JK) - PDRUS_TURB(JI,JJ,JK)
        PDRVS_TURB(JI,JJ,JK)   = PRVS(JI,JJ,JK) - PDRVS_TURB(JI,JJ,JK)
        PDRTHLS_TURB(JI,JJ,JK) = PRTHLS(JI,JJ,JK) - PDRTHLS_TURB(JI,JJ,JK)
        PDRRTS_TURB(JI,JJ,JK)  = PRRS(JI,JJ,JK,1) - PDRRTS_TURB(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
  DO JSV=1,KSV 
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE   
          PDRSVS_TURB(JI,JJ,JK,JSV)  = PRSVS(JI,JJ,JK,JSV) - PDRSVS_TURB(JI,JJ,JK,JSV)
        ENDDO
      ENDDO
    ENDDO
  ENDDO
...
!
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        PLOCPEXN(JI,JJ,JK) = ( PLTT + (CST%XCPV-PC) *  (PT(JI,JJ,JK)-CST%XTT) ) &
        / PCP(JI,JJ,JK)
!
!*      1.2 Saturation vapor pressure at t
!
        ZRVSAT(JI,JJ,JK) =  EXP( PALP - PBETA/PT(JI,JJ,JK) - PGAM*ALOG( PT(JI,JJ,JK) ) )
!
!*      1.3 saturation  mixing ratio at t
!
        ZRVSAT(JI,JJ,JK) =  ZRVSAT(JI,JJ,JK) &
        * ZEPS / ( PPABST(JI,JJ,JK) - ZRVSAT(JI,JJ,JK) )
!
!*      1.4 compute the saturation mixing ratio derivative (rvs')
!
        ZDRVSATDT(JI,JJ,JK) = ( PBETA / PT(JI,JJ,JK)  - PGAM ) / PT(JI,JJ,JK)   &
        * ZRVSAT(JI,JJ,JK) * ( 1. + ZRVSAT(JI,JJ,JK) / ZEPS )
!
!*      1.5 compute Amoist
!
        PAMOIST(JI,JJ,JK)=  0.5 / ( 1.0 + ZDRVSATDT(JI,JJ,JK) * PLOCPEXN(JI,JJ,JK) )
!
!*      1.6 compute Atheta
!
        ...
        (                                                                  &
        ZRVSAT(JI,JJ,JK) * (1. + ZRVSAT(JI,JJ,JK)/ZEPS)                         &
        * ( -2.*PBETA/PT(JI,JJ,JK) + PGAM ) / PT(JI,JJ,JK)**2      &
