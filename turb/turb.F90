...
DO JK=1,D%NKT 
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
PTP(JI,JJ,JK) = PTP(JI,JJ,JK) + CST%XG / PTHVREF(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
  ENDDO
 ENDDO
ENDDO
...
!
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
  PLOCPEXN(JI,JJ,JK) = ( PLTT + (CST%XCPV-PC) *  (PT(JI,JJ,JK)-CST%XTT) ) / PCP(JI,JJ,JK)
!
...
!
  ZRVSAT(JI,JJ,JK) =  ZRVSAT(JI,JJ,JK) * ZEPS / ( PPABST(JI,JJ,JK) - ZRVSAT(JI,JJ,JK) )
!
