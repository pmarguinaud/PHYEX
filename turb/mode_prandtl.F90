...
DO JJ=IJB,IJE 
  DO JI=IIB,IIE 
    PETHETA(JI,JJ,D%NKA) = 2.*PETHETA(JI,JJ,IKB) - PETHETA(JI,JJ,IKB+D%NKL)
    PEMOIST(JI,JJ,D%NKA) = 2.*PEMOIST(JI,JJ,IKB) - PEMOIST(JI,JJ,IKB+D%NKL)
  ENDDO
ENDDO
!
!---------------------------------------------------------------------------
...
!
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK) = CST%XG / PTHVREF(JI,JJ,JK) * PLM(JI,JJ,JK) & 
        * PLEPS(JI,JJ,JK) / PTKEM(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
...
IF (KRR /= 0) THEN                ! moist case
  CALL GZ_M_W_PHY(D,PRM(:,:,:,1),PDZZ,ZWORK2)
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PREDTH1(JI,JJ,JK)= CSTURB%XCTV*PBLL_O_E(JI,JJ,JK) * PETHETA(JI,JJ,JK) &
          * ZWORK1(JI,JJ,JK)
          PREDR1(JI,JJ,JK) = CSTURB%XCTV*PBLL_O_E(JI,JJ,JK) * PEMOIST(JI,JJ,JK) &
          * ZWORK2(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
ELSE                              ! dry case
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PREDTH1(JI,JJ,JK)= CSTURB%XCTV*PBLL_O_E(JI,JJ,JK)  * ZWORK1(JI,JJ,JK)
          PREDR1(JI,JJ,JK) = 0.
        ENDDO
      ENDDO
    ENDDO
...
DO JSV=1,KSV
  CALL GZ_M_W_PHY(D,PSVM(:,:,:,JSV),PDZZ,ZWORK1)
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PREDS1(JI,JJ,JK,JSV)=CSTURB%XCTV*PBLL_O_E(JI,JJ,JK)*ZWORK1(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
END DO
...
!
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PRED2TH3(JI,JJ,JK)  = PREDTH1(JI,JJ,JK)**2
!
          PRED2R3(JI,JJ,JK)   = PREDR1(JI,JJ,JK) **2
!
          PRED2THR3(JI,JJ,JK) = PREDTH1(JI,JJ,JK) * PREDR1(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
!
...
!
      DO JK=1,D%NKT 
        DO JJ=1,D%NJT 
          DO JI=1,D%NIT 
            PRED2TH3(JI,JJ,JK)= PREDTH1(JI,JJ,JK)**2+(CSTURB%XCTV*PBLL_O_E(JI,JJ,JK)*PETHETA(JI,JJ,JK) )**2 * &
            ZWORK1(JI,JJ,JK)
!
            PRED2R3(JI,JJ,JK)= PREDR1(JI,JJ,JK)**2 + (CSTURB%XCTV*PBLL_O_E(JI,JJ,JK)*PEMOIST(JI,JJ,JK))**2 * &
            ZWORK2(JI,JJ,JK)
!
            PRED2THR3(JI,JJ,JK)= PREDR1(JI,JJ,JK) * PREDTH1(JI,JJ,JK) +  CSTURB%XCTV**2*PBLL_O_E(JI,JJ,JK)**2 *   &
            PEMOIST(JI,JJ,JK) * PETHETA(JI,JJ,JK) *                         &
            ZWORK3(JI,JJ,JK)
          ENDDO
        ENDDO
      ENDDO
...
!
  ELSE                 ! dry 3D case in a 2D model
      DO JK=1,D%NKT 
        DO JJ=1,D%NJT 
          DO JI=1,D%NIT 
            PRED2TH3(JI,JJ,JK) = PREDTH1(JI,JJ,JK)**2 +  CSTURB%XCTV**2*PBLL_O_E(JI,JJ,JK)**2 *     &
            ZWORK1(JI,JJ,JK)      
          ENDDO
        ENDDO
      ENDDO
...
!
            PRED2THR3(JI,JJ,JK)= PREDR1(JI,JJ,JK) * PREDTH1(JI,JJ,JK) +  CSTURB%XCTV**2*PBLL_O_E(JI,JJ,JK)**2 *   &
            PEMOIST(JI,JJ,JK) * PETHETA(JI,JJ,JK) * ZWORK3(JI,JJ,JK)
...
!        1D case
      DO JK=1,D%NKT 
        DO JJ=IJB,IJE 
          DO JI=IIB,IIE 
            PRED2THS3(JI,JJ,JK,JSV)  = PREDS1(JI,JJ,JK,JSV) * PREDTH1(JI,JJ,JK)
            IF (KRR /= 0) THEN
              PRED2RS3(JI,JJ,JK,JSV)   = PREDR1(JI,JJ,JK) *PREDS1(JI,JJ,JK,JSV)
            ELSE
              PRED2RS3(JI,JJ,JK,JSV)   = 0.
            END IF
          ENDDO
        ENDDO
      ENDDO
...
!
      DO JK=1,D%NKT 
        DO JJ=1,D%NJT 
          DO JI=1,D%NIT     
            IF (KRR /= 0) THEN
              ZWORK1(JI,JJ,JK) = ZW1(JI,JJ,JK)*PETHETA(JI,JJ,JK)
            ELSE
              ZWORK1(JI,JJ,JK) = ZW1(JI,JJ,JK)
            END IF
            PRED2THS3(JI,JJ,JK,JSV) = PREDTH1(JI,JJ,JK) * PREDS1(JI,JJ,JK,JSV)   +        &
            ZWORK1(JI,JJ,JK) * ZWORK2(JI,JJ,JK)
                       !
            IF (KRR /= 0) THEN
              PRED2RS3(JI,JJ,JK,JSV) = PREDR1(JI,JJ,JK) * PREDS1(JI,JJ,JK,JSV)   +        &
              ZW1(JI,JJ,JK) * PEMOIST(JI,JJ,JK) * ZWORK3(JI,JJ,JK)
            ELSE
              PRED2RS3(JI,JJ,JK,JSV) = 0.
            END IF
          ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE     
      ZCOEF(JI,JJ,JK) = MAX(MIN((  10.*(1.-PPHI3(JI,JJ,JK)/CSTURB%XPHI_LIM)) ,1.), 0.) 
!
      PF(JI,JJ,JK) =     ZCOEF(JI,JJ,JK)   * PF(JI,JJ,JK)    &
      + (1.-ZCOEF(JI,JJ,JK))  * PF_LIM(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO    
!
...
IF (HTURBDIM=='3DIM') THEN
    ...
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE     
        IF (OUSERV) THEN
          ...
          ZW2(JI,JJ,JK) = 0.5 * (PRED2TH3(JI,JJ,JK)-PRED2R3(JI,JJ,JK))
          
          PHI3(JI,JJ,JK)= 1. -                                          &
          ( ( (1.+PREDR1(JI,JJ,JK)) *                                   &
          (PRED2THR3(JI,JJ,JK) + PRED2TH3(JI,JJ,JK)) / PREDTH1(JI,JJ,JK)  &
          ) + ZW2(JI,JJ,JK)                                           &
          ) / ZW1(JI,JJ,JK)
        ELSE
          ...
          ZW2(JI,JJ,JK) = 0.5* PRED2TH3(JI,JJ,JK)
          
          PHI3(JI,JJ,JK)= 1. -                                       &
          (PRED2TH3(JI,JJ,JK) / PREDTH1(JI,JJ,JK) + ZW2(JI,JJ,JK)) &
          / ZW1(JI,JJ,JK)
        END IF
      ENDDO
    ENDDO
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE     
        IF( PHI3(JI,JJ,JK) <= 0. .OR. PHI3(JI,JJ,JK) > CSTURB%XPHI_LIM )THEN
          PHI3(JI,JJ,JK) = CSTURB%XPHI_LIM
        ENDIF
      ENDDO
    ENDDO
  ENDDO    
ELSE
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        IF (OUSERV) THEN
          PHI3(JI,JJ,JK)= 1./(1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))
        ELSE
          PHI3(JI,JJ,JK)= 1./(1.+PREDTH1(JI,JJ,JK))
        END IF
      ENDDO
    ENDDO
  ENDDO    
...
DO JSV=1,KSV
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE     
        PSI_SV(JI,JJ,JK,JSV) = ( 1.                                             &
        - (CSTURB%XCPR3+CSTURB%XCPR5) * &
        (PRED2THS(JI,JJ,JK,JSV)/PREDS1(JI,JJ,JK,JSV)-PREDTH1(JI,JJ,JK)) &
        - (CSTURB%XCPR4+CSTURB%XCPR5) * &
        (PRED2RS(JI,JJ,JK,JSV)/PREDS1(JI,JJ,JK,JSV)-PREDR1(JI,JJ,JK)) &
        - CSTURB%XCPR3 * &
        PREDTH1(JI,JJ,JK) * PPHI3(JI,JJ,JK) &
        - CSTURB%XCPR4 * PREDR1(JI,JJ,JK) * PPSI3(JI,JJ,JK)                 &
        ) / (  1. + CSTURB%XCPR5 * ( PREDTH1(JI,JJ,JK) + PREDR1(JI,JJ,JK) ) )           
...
!        control of the PSI_SV positivity
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE     
        IF ( (PSI_SV(JI,JJ,JK,JSV) <=0.).AND. (PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))<=0.)THEN
          PSI_SV(JI,JJ,JK,JSV)=CSTURB%XPHI_LIM
        ENDIF
      ENDDO
    ENDDO
  ENDDO    
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE     
        PSI_SV(JI,JJ,JK,JSV) = MAX( 1.E-4, MIN(CSTURB%XPHI_LIM,PSI_SV(JI,JJ,JK,JSV)) )
      ENDDO
    ENDDO
  ENDDO    
!
  ...
  IF (OUSERV) THEN
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE     
#ifdef REPRO48
          IF (PPHI3(JI,JJ,JK)/=CSTURB%XPHI_LIM)THEN
#else
          IF (PPHI3(JI,JJ,JK)<=CSTURB%XPHI_LIM)THEN
#endif
            D_PHI3DTDZ_O_DDTDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)                       &
            * (1. - PREDTH1(JI,JJ,JK) * (3./2.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))   &
            /((1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)) &
            *(1.+1./2.*(PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))))) &
            + (1.+PREDR1(JI,JJ,JK))*(PRED2THR3(JI,JJ,JK)+PRED2TH3(JI,JJ,JK))         &
            / (PREDTH1(JI,JJ,JK)*(1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))* &
            (1.+1./2.*(PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)))) &
            - (1./2.*PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK) &
            * (1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)))           &
            / ((1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))&
            *(1.+1./2.*(PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))))
          ELSE
...
#endif
            D_PHI3DTDZ_O_DDTDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)             &
            * (1. - PREDTH1(JI,JJ,JK) * (3./2.+PREDTH1(JI,JJ,JK))      &
            /((1.+PREDTH1(JI,JJ,JK))*(1.+1./2.*PREDTH1(JI,JJ,JK))))        &
            + PRED2TH3(JI,JJ,JK) &
            / (PREDTH1(JI,JJ,JK)*(1.+PREDTH1(JI,JJ,JK))*(1.+1./2.*PREDTH1(JI,JJ,JK))) &
            - 1./2.*PREDTH1(JI,JJ,JK) &
            / ((1.+PREDTH1(JI,JJ,JK))*(1.+1./2.*PREDTH1(JI,JJ,JK)))
          ELSE
            D_PHI3DTDZ_O_DDTDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)
          ENDIF
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE     
          ...
          IF (PPHI3(JI,JJ,JK)<=CSTURB%XPHI_LIM)THEN
#endif
            D_PHI3DRDZ_O_DDRDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK) &
            * (1.-PREDR1(JI,JJ,JK)*(3./2.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)) &
            / ((1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)) & 
            *(1.+1./2.*(PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)))))  &
            - PREDR1(JI,JJ,JK) &
            * (PRED2THR3(JI,JJ,JK)+PRED2TH3(JI,JJ,JK)) / (PREDTH1(JI,JJ,JK)         &
            * (1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))*&
            (1.+1./2.*(PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))))    &
            + PREDR1(JI,JJ,JK) * (1./2.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))         &
            / ((1.+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))&
            *(1.+1./2.*(PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))))
          ELSE
            D_PHI3DRDZ_O_DDRDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)
...
#endif
          D_PHI3DRDZ_O_DDRDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)                           &
          * (1. - PREDR1(JI,JJ,JK)*PPHI3(JI,JJ,JK))
        ELSE
          D_PHI3DRDZ_O_DDRDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)
        ENDIF
      ENDDO
...
IF (HTURBDIM=='3DIM') THEN
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        D_PHI3DTDZ2_O_DDTDZ(JI,JJ,JK) = PDTDZ(JI,JJ,JK) &
        * (PPHI3(JI,JJ,JK) +  ZWORK1(JI,JJ,JK))
      ENDDO
    ENDDO
  ENDDO
ELSE
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE     
        ...
        IF (PPHI3(JI,JJ,JK)<=CSTURB%XPHI_LIM)THEN
#endif
          D_PHI3DTDZ2_O_DDTDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK)*PDTDZ(JI,JJ,JK)             &
          * (2. - PREDTH1(JI,JJ,JK)*PPHI3(JI,JJ,JK))
        ELSE
          D_PHI3DTDZ2_O_DDTDZ(JI,JJ,JK) = PPHI3(JI,JJ,JK) * 2. * PDTDZ(JI,JJ,JK)
        ENDIF
      ENDDO
    ENDDO
  ENDDO    
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_WTH_WTH2(JI,JJ,JK) = CSTURB%XCSHF*PBLL_O_E(JI,JJ,JK)&
      * PETHETA(JI,JJ,JK)*0.5/CSTURB%XCTD        &
      * (1.+0.5*PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK)) / PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_WTH_WTH2_O_DDTDZ(JI,JJ,JK) = &
      (0.5*CSTURB%XCSHF*PBLL_O_E(JI,JJ,JK)*PETHETA(JI,JJ,JK)*0.5/CSTURB%XCTD/PD(JI,JJ,JK) &
      - PM3_WTH_WTH2(JI,JJ,JK)/PD(JI,JJ,JK)&
      *(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))  )&
      * PBLL_O_E(JI,JJ,JK) * PETHETA(JI,JJ,JK) * CSTURB%XCTV
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_WTH_W2TH(JI,JJ,JK) = CSTURB%XCSHF*PKEFF(JI,JJ,JK)*1.5/ZWORK1(JI,JJ,JK) &
      * (1. - 0.5*PREDR1(JI,JJ,JK)*(1.+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK) ) &
      / (1.+PREDTH1(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_WTH_W2TH_O_DDTDZ(JI,JJ,JK) = &
      - CSTURB%XCSHF*PKEFF(JI,JJ,JK)*1.5/ZWORK1(JI,JJ,JK)/(1.+PREDTH1(JI,JJ,JK))**2 &
      * CSTURB%XCTV*PBLL_O_E(JI,JJ,JK)*PETHETA(JI,JJ,JK)  &
      * (1. - 0.5*PREDR1(JI,JJ,JK)*(1.+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)* &
      ( 1.+(1.+PREDTH1(JI,JJ,JK))*(1.5+PREDR1(JI,JJ,JK)+PREDTH1(JI,JJ,JK))&
      /PD(JI,JJ,JK)) )
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_WTH_W2R(JI,JJ,JK) = &
      - CSTURB%XCSHF*PKEFF(JI,JJ,JK)*0.75*CSTURB%XCTV*PBLL_O_E(JI,JJ,JK) &
      /ZWORK1(JI,JJ,JK)*PEMOIST(JI,JJ,JK)*PDTDZ(JI,JJ,JK)/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_WTH_W2R_O_DDTDZ(JI,JJ,JK) = &
      - CSTURB%XCSHF*PKEFF(JI,JJ,JK)*0.75*CSTURB%XCTV*PBLL_O_E(JI,JJ,JK) &
      /ZWORK1(JI,JJ,JK)*PEMOIST(JI,JJ,JK)/PD(JI,JJ,JK) &
      * (1. -  PREDTH1(JI,JJ,JK)*(1.5+PREDTH1(JI,JJ,JK)& 
      +PREDR1(JI,JJ,JK))/PD(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PBETA(JI,JJ,JK)*PLEPS(JI,JJ,JK) &
      /(PSQRT_TKE(JI,JJ,JK)*PTKE(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_WTH_WR2(JI,JJ,JK) = - CSTURB%XCSHF*PKEFF(JI,JJ,JK)& 
      *0.25*PBLL_O_E(JI,JJ,JK)*CSTURB%XCTV*PEMOIST(JI,JJ,JK)**2 &
      *ZWORK2(JI,JJ,JK)/CSTURB%XCTD*PDTDZ(JI,JJ,JK)/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_WTH_WR2_O_DDTDZ(JI,JJ,JK) = - CSTURB%XCSHF*PKEFF(JI,JJ,JK)& 
      *0.25*PBLL_O_E(JI,JJ,JK)*CSTURB%XCTV*PEMOIST(JI,JJ,JK)**2 &
      *ZWORK2(JI,JJ,JK)/CSTURB%XCTD/PD(JI,JJ,JK)     &
      * (1. -  PREDTH1(JI,JJ,JK)* &
      (1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_WTH_WTHR(JI,JJ,JK) = &
      CSTURB%XCSHF*PKEFF(JI,JJ,JK)*PEMOIST(JI,JJ,JK)*ZWORK2(JI,JJ,JK) &
      *0.5*PLEPS(JI,JJ,JK)/CSTURB%XCTD*(1+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_WTH_WTHR_O_DDTDZ(JI,JJ,JK) = &
      - PM3_WTH_WTHR(JI,JJ,JK) * (1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))&
      /PD(JI,JJ,JK)*CSTURB%XCTV*PBLL_O_E(JI,JJ,JK)*PETHETA(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = (1.-0.5*PREDR1(JI,JJ,JK)*(1.+PREDR1(JI,JJ,JK))& 
      /PD(JI,JJ,JK))/(1.+PREDTH1(JI,JJ,JK))*PDTDZ(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_TH2_W2TH(JI,JJ,JK) = - ZWORK2(JI,JJ,JK) &
      * 1.5*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)/PTKE(JI,JJ,JK)*CSTURB%XCTV
    ENDDO
  ENDDO
ENDDO
!
...
!        / (1.+PREDTH1)**2, D%NKA, D%NKU, D%NKL)
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK) = (1.-0.5*PREDR1(JI,JJ,JK)*(1.+PREDR1(JI,JJ,JK))&
        / PD(JI,JJ,JK))*(1.-(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))   &
        * PREDTH1(JI,JJ,JK)*(1.+PREDTH1(JI,JJ,JK))/PD(JI,JJ,JK) ) &
        / (1.+PREDTH1(JI,JJ,JK))**2
      ENDDO
    ENDDO
  ENDDO
  CALL MZF_PHY(D,ZWORK1,ZWORK2)
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        D_M3_TH2_W2TH_O_DDTDZ(JI,JJ,JK) = - 1.5*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK) &
        /PTKE(JI,JJ,JK)*CSTURB%XCTV * ZWORK2(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
ELSE
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK) = 1./(1.+PREDTH1(JI,JJ,JK))**2
      ENDDO
    ENDDO
  ENDDO
  CALL MZF_PHY(D,ZWORK1,ZWORK2)
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = (1.+0.5*PREDTH1(JI,JJ,JK) &
      +1.5*PREDR1(JI,JJ,JK)+0.5*PREDR1(JI,JJ,JK)**2)/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_TH2_WTH2(JI,JJ,JK) = PLEPS(JI,JJ,JK)*0.5/CSTURB%XCTD/PSQRT_TKE(JI,JJ,JK) &
      * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PBLL_O_E(JI,JJ,JK)*PETHETA(JI,JJ,JK) &
      * (0.5/PD(JI,JJ,JK) - (1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))& 
      *(1.+0.5*PREDTH1(JI,JJ,JK)+1.5*PREDR1(JI,JJ,JK)& 
      +0.5*PREDR1(JI,JJ,JK)**2)/PD(JI,JJ,JK)**2)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_TH2_WTH2_O_DDTDZ(JI,JJ,JK) = PLEPS(JI,JJ,JK) & 
      *0.5/CSTURB%XCTD/PSQRT_TKE(JI,JJ,JK)*CSTURB%XCTV * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PBLL_O_E(JI,JJ,JK)*PEMOIST(JI,JJ,JK) & 
      /PD(JI,JJ,JK)*PDTDZ(JI,JJ,JK)**2
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_TH2_W2R(JI,JJ,JK) = 0.75*CSTURB%XCTV**2*ZWORK2(JI,JJ,JK) &
      *PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)/PTKE(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) =  PBLL_O_E(JI,JJ,JK)*PEMOIST(JI,JJ,JK)& 
      /PD(JI,JJ,JK)*PDTDZ(JI,JJ,JK)*(2.-PREDTH1(JI,JJ,JK)* & 
      (1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_TH2_W2R_O_DDTDZ(JI,JJ,JK) = 0.75*CSTURB%XCTV**2*PLM(JI,JJ,JK) *PLEPS(JI,JJ,JK) &
      /PTKE(JI,JJ,JK) * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_TH2_WR2(JI,JJ,JK) = 0.25*CSTURB%XCTV**2*ZWORK2(JI,JJ,JK)&
      *PLEPS(JI,JJ,JK)/PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = (PBLL_O_E(JI,JJ,JK)*PEMOIST(JI,JJ,JK))**2 & 
      *PDTDZ(JI,JJ,JK)/PD(JI,JJ,JK)*(2.-PREDTH1(JI,JJ,JK) & 
      *(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_TH2_WR2_O_DDTDZ(JI,JJ,JK) = 0.25*CSTURB%XCTV**2*PLEPS(JI,JJ,JK) & 
      / PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PBLL_O_E(JI,JJ,JK)*PEMOIST(JI,JJ,JK) & 
      * PDTDZ(JI,JJ,JK)*(1.+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_TH2_WTHR(JI,JJ,JK) = - 0.5*CSTURB%XCTV*PLEPS(JI,JJ,JK) & 
      / PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PBLL_O_E(JI,JJ,JK)*PEMOIST(JI,JJ,JK)* & 
      (1.+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK) * (1. -PREDTH1(JI,JJ,JK)*& 
      (1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_TH2_WTHR_O_DDTDZ(JI,JJ,JK) = - 0.5*CSTURB%XCTV*PLEPS(JI,JJ,JK) & 
      / PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD * ZWORK2(JI,JJ,JK) 
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) =  (1.+PREDTH1(JI,JJ,JK))* & 
      (1.+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_THR_WTHR(JI,JJ,JK) = 0.5*PLEPS(JI,JJ,JK)/PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD &
      * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PETHETA(JI,JJ,JK)*PBLL_O_E(JI,JJ,JK)/PD(JI,JJ,JK) & 
      *(1.+PREDR1(JI,JJ,JK))*(1.-(1.+PREDTH1(JI,JJ,JK)) & 
      *(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_THR_WTHR_O_DDTDZ(JI,JJ,JK) = 0.5*PLEPS(JI,JJ,JK)/PSQRT_TKE(JI,JJ,JK) & 
      / CSTURB%XCTD * CSTURB%XCTV * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = (1.+PREDR1(JI,JJ,JK))*PBLL_O_E(JI,JJ,JK)* & 
      PETHETA(JI,JJ,JK)*PDRDZ(JI,JJ,JK)/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_THR_WTH2(JI,JJ,JK) = - 0.25*PLEPS(JI,JJ,JK) & 
      / PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD*CSTURB%XCTV * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = -(1.+PREDR1(JI,JJ,JK))*(PBLL_O_E(JI,JJ,JK) & 
      *PETHETA(JI,JJ,JK)/PD(JI,JJ,JK))**2&
      *PDRDZ(JI,JJ,JK)&
      *(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))
    ENDDO
  ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_THR_WTH2_O_DDTDZ(JI,JJ,JK) = - 0.25*PLEPS(JI,JJ,JK) &
      /PSQRT_TKE(JI,JJ,JK)/CSTURB%XCTD*CSTURB%XCTV**2 * ZWORK2(JI,JJ,JK) 
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = PBLL_O_E(JI,JJ,JK)*PETHETA(JI,JJ,JK)/PD(JI,JJ,JK)&
      *(-(1.+PREDR1(JI,JJ,JK))*PREDR1(JI,JJ,JK)/PD(JI,JJ,JK)&
      *(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))+(1.+2.*PREDR1(JI,JJ,JK)))
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_THR_WTH2_O_DDRDZ(JI,JJ,JK) = - 0.25*PLEPS(JI,JJ,JK)/PSQRT_TKE(JI,JJ,JK)& 
      / CSTURB%XCTD*CSTURB%XCTV * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) = (1.+PREDR1(JI,JJ,JK))*PDRDZ(JI,JJ,JK)/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      M3_THR_W2TH(JI,JJ,JK) = - 0.75*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)& 
      / PTKE(JI,JJ,JK) * CSTURB%XCTV * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) =  -PETHETA(JI,JJ,JK)*PBLL_O_E(JI,JJ,JK)*& 
      (1.+PREDR1(JI,JJ,JK))*PDRDZ(JI,JJ,JK)& 
      *(1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)**2
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_THR_W2TH_O_DDTDZ(JI,JJ,JK) = - 0.75*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)&
      / PTKE(JI,JJ,JK) * CSTURB%XCTV**2 * ZWORK1(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK1(JI,JJ,JK) =  -(1.+PREDR1(JI,JJ,JK))*PREDR1(JI,JJ,JK)&
      * (1.5+PREDTH1(JI,JJ,JK)+PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)**2          &
      +(1.+2.*PREDR1(JI,JJ,JK))/PD(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
CALL MZF_PHY(D,ZWORK1,ZWORK2)
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      D_M3_THR_W2TH_O_DDRDZ(JI,JJ,JK) = - 0.75*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)&
      / PTKE(JI,JJ,JK) * CSTURB%XCTV * ZWORK2(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
