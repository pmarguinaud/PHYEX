...
ELSE
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
  ZF(JI,JJ,JK) = -CSTURB%XCSHF*PPHI3(JI,JJ,JK)*ZKEFF(JI,JJ,JK)*ZWORK1(JI,JJ,JK)/PDZZ(JI,JJ,JK)
...
DO JK=1,D%NKT 
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT     
ZRWTHL(JI,JJ,JK)= PRHODJ(JI,JJ,JK)*(PTHLP(JI,JJ,JK)-PTHLM(JI,JJ,JK))/PTSTEP
  ENDDO
 ENDDO
ENDDO    
...
!
!
ZFLXZ(JI,JJ,JK)   = ZF(JI,JJ,JK) + PIMPL * ZDFDDTDZ(JI,JJ,JK) * ZWORK1(JI,JJ,JK)/ PDZZ(JI,JJ,JK)
  ENDDO
 ENDDO
ENDDO
!
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
     PRRS(JI,JJ,JK,2) = PRRS(JI,JJ,JK,2) -                                        &
                     PRHODJ(JI,JJ,JK)*PATHETA(JI,JJ,JK)*2.*PSRCM(JI,JJ,JK)*ZWORK1(JI,JJ,JK) &
...
ENDDO    
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT     
  ZF(JI,JJ,JK) = -CSTURB%XCSHF*PPSI3(JI,JJ,JK)*ZKEFF(JI,JJ,JK)*ZWORK1(JI,JJ,JK)/PDZZ(JI,JJ,JK)
...
ENDDO 
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT     
  ZRWRNP(JI,JJ,JK) = PRHODJ(JI,JJ,JK)*(PRP(JI,JJ,JK)-PRM(JI,JJ,JK,1))/PTSTEP
    ENDDO
 ENDDO
ENDDO    
  !
...
ENDDO
 ...
 DO JJ=1,D%NJT 
  ...
  ZFLXZ(JI,JJ,JK)   = ZF(JI,JJ,JK)                                                &
                 + PIMPL * ZDFDDRDZ(JI,JJ,JK) * ZWORK1(JI,JJ,JK) / PDZZ(JI,JJ,JK) 
    ENDDO
 ENDDO
ENDDO
  !
...
ENDDO
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
    ZA(JI,JJ,JK)   =  PBETA(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
      ENDDO
 ENDDO
ENDDO
    DO JJ=1,D%NJT 
...
IF(HPROGRAM/='AROME  ') THEN
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
       PRRS(JI,JJ,JK,2) = PRRS(JI,JJ,JK,2) -                                        &
                       PRHODJ(JI,JJ,JK)*PAMOIST(JI,JJ,JK)*2.*PSRCM(JI,JJ,JK)*ZWORK1(JI,JJ,JK)       &
                       *(1.0-PFRAC_ICE(JI,JJ,JK))
       PRRS(JI,JJ,JK,4) = PRRS(JI,JJ,JK,4) -                                        &
                       PRHODJ(JI,JJ,JK)*PAMOIST(JI,JJ,JK)*2.*PSRCM(JI,JJ,JK)*ZWORK1(JI,JJ,JK)       &
                       *PFRAC_ICE(JI,JJ,JK)
...
! yet modify to use length scale and tke at half levels from vdfexcuhl
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
  ZA(JI,JJ,JK)   = ZWORK1(JI,JJ,JK)/ PDZZ(JI,JJ,JK) * (-PLM(JI,JJ,JK)*PSQRT_TKE(JI,JJ,JK))
    ENDDO
 ENDDO
ENDDO
 ELSE
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
  ZA(JI,JJ,JK)   = ZWORK1(JI,JJ,JK)/ PDZZ(JI,JJ,JK) * (-PPHI3(JI,JJ,JK)*ZWORK2(JI,JJ,JK)) * CSTURB%XCSHF 
    ENDDO
 ENDDO
ENDDO
 ENDIF
...
ENDDO
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
  ZFLXZ(JI,JJ,JK) = ZWORK1(JI,JJ,JK)* ZFLXZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK)* ZA(JI,JJ,JK)
    ENDDO
 ENDDO
ENDDO
  DO JJ=1,D%NJT 
