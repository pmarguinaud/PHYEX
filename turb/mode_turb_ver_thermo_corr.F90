...
DO JJ=IJB,IJE 
  DO JI=IIB,IIE 
    ZCOEFF(JI,JJ,IKB+2*D%NKL)= - PDZZ(JI,JJ,IKB+D%NKL) /      &
    ( (PDZZ(JI,JJ,IKB+2*D%NKL)+PDZZ(JI,JJ,IKB+D%NKL)) * PDZZ(JI,JJ,IKB+2*D%NKL) )
    ZCOEFF(JI,JJ,IKB+D%NKL)=   (PDZZ(JI,JJ,IKB+2*D%NKL)+PDZZ(JI,JJ,IKB+D%NKL)) /      &
    ( PDZZ(JI,JJ,IKB+D%NKL) * PDZZ(JI,JJ,IKB+2*D%NKL) )
    ZCOEFF(JI,JJ,IKB)= - (PDZZ(JI,JJ,IKB+2*D%NKL)+2.*PDZZ(JI,JJ,IKB+D%NKL)) /      &
    ( (PDZZ(JI,JJ,IKB+2*D%NKL)+PDZZ(JI,JJ,IKB+D%NKL)) * PDZZ(JI,JJ,IKB+D%NKL) )
  ENDDO
ENDDO
!
!
IF (OHARAT) THEN
  ...
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      PLMF(JI,JJ,D%NKT)=0.001
      PLEPSF(JI,JJ,D%NKT)=0.001
    ENDDO
  ENDDO
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
...
!
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK)=PDTH_DZ(JI,JJ,JK)**2
      ENDDO
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZF(JI,JJ,JK) = PLMF(JI,JJ,JK)*PLEPSF(JI,JJ,JK)*ZWORK2(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
  ELSE
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK)=PPHI3(JI,JJ,JK)*PDTH_DZ(JI,JJ,JK)**2
      ENDDO
    ENDDO
  ENDDO
    CALL MZF_PHY(D,ZWORK1,ZWORK2)
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZF(JI,JJ,JK) = CSTURB%XCTV*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)& 
        * ZWORK2(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JJ=1,D%NJT 
      DO JI=1,D%NIT 
        ZF(JI,JJ,JK)       = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFTH2(JI,JJ,JK)
        ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFTH2(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
  END IF
  ...
  IF (KRR/=0) THEN
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFR2(JI,JJ,JK)
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFR2(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * ZWORK2(JI,JJ,JK)
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK3(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFTHR(JI,JJ,JK)
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFTHR(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZWORK3(JI,JJ,JK) = ZWORK2(JI,JJ,JK) / PDZZ(JI,JJ,JK)
    ENDDO
  ENDDO
ENDDO
...
ENDDO
  ...
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZFLXZ(JI,JJ,IKB) =  PLMF(JI,JJ,IKB)   &
      * PLEPSF(JI,JJ,IKB)                                         &
      *( PEXPL *                                                  &
      ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PTHLM(JI,JJ,IKB+2*D%NKL)             &
      +ZCOEFF(JI,JJ,IKB+D%NKL)*PTHLM(JI,JJ,IKB+D%NKL)             & 
      +ZCOEFF(JI,JJ,IKB)*PTHLM(JI,JJ,IKB)   )**2          &
      +PIMPL *                                                  &
      ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PTHLP(JI,JJ,IKB+2*D%NKL)             &
      +ZCOEFF(JI,JJ,IKB+D%NKL)*PTHLP(JI,JJ,IKB+D%NKL)             &
      +ZCOEFF(JI,JJ,IKB)*PTHLP(JI,JJ,IKB)   )**2          &
      ) 
    ENDDO
  ENDDO
   ELSE
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZFLXZ(JI,JJ,IKB) = CSTURB%XCTV * PPHI3(JI,JJ,IKB+D%NKL) * PLM(JI,JJ,IKB)   &
      * PLEPS(JI,JJ,IKB)                                         &
      *( PEXPL *                                                  &
      ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PTHLM(JI,JJ,IKB+2*D%NKL)             &
      +ZCOEFF(JI,JJ,IKB+D%NKL)*PTHLM(JI,JJ,IKB+D%NKL)             & 
...
DO JK=1,D%NKT 
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      ZFLXZ(JI,JJ,JK) = MAX(0., ZFLXZ(JI,JJ,JK))
    ENDDO
  ENDDO
ENDDO
  !
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        PSIGS(JI,JJ,JK) = ZFLXZ(JI,JJ,JK) * PATHETA(JI,JJ,JK)**2
      ENDDO
    ENDDO
  ENDDO
  END IF
...
!
  ...
  IF (OHARAT) THEN
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK1(JI,JJ,JK) = PDTH_DZ(JI,JJ,JK)*PDR_DZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    CALL MZF_PHY(D,ZWORK1,ZWORK2)
  ...
  ELSE
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK1(JI,JJ,JK) = 0.5*(PPHI3(JI,JJ,JK)+PPSI3(JI,JJ,JK))& 
          *PDTH_DZ(JI,JJ,JK)*PDR_DZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    CALL MZF_PHY(D,ZWORK1,ZWORK2)
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFTH2(JI,JJ,JK)
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFTH2(JI,JJ,JK)
          ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK3(JI,JJ,JK) * PFTH2(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ...
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK3(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
          ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK4(JI,JJ,JK) * ZWORK1(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFR2(JI,JJ,JK)
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFR2(JI,JJ,JK)
          ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK3(JI,JJ,JK) * PFR2(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFTHR(JI,JJ,JK)
          ZDFDDTDZ(JI,JJ,JK) = ZDFDDTDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFTHR(JI,JJ,JK)
          ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK3(JI,JJ,JK) * PFTHR(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK) = PTHLP(JI,JJ,JK) - PTHLM(JI,JJ,JK)
        ZWORK2(JI,JJ,JK) = PRP(JI,JJ,JK) - PRM(JI,JJ,JK,1)
      ENDDO
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZWORK1(JI,JJ,JK) = ZWORK3(JI,JJ,JK) / PDZZ(JI,JJ,JK)
        ZWORK2(JI,JJ,JK) = ZWORK4(JI,JJ,JK) / PDZZ(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ...
          + PIMPL * PLMF(JI,JJ,JK)*PLEPSF(JI,JJ,JK)*0.5                          &
          * ZWORK5(JI,JJ,JK)                                                &
          + PIMPL * ZDFDDTDZ(JI,JJ,JK) * ZWORK7(JI,JJ,JK)         &
          + PIMPL * ZDFDDRDZ(JI,JJ,JK) * ZWORK8(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    ELSE
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK5(JI,JJ,JK) = (ZWKPHIPSI1(JI,JJ,JK)+ZWKPHIPSI2(JI,JJ,JK))& 
          *PDR_DZ(JI,JJ,JK)*ZWORK3(JI,JJ,JK)/PDZZ(JI,JJ,JK) &
          + (ZWKPHIPSI3(JI,JJ,JK) + ZWKPHIPSI4(JI,JJ,JK)) & 
          *PDTH_DZ(JI,JJ,JK)*ZWORK4(JI,JJ,JK)/PDZZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
      CALL MZF_PHY(D,ZWORK5,ZWORK6)      
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZFLXZ(JI,JJ,JK)   = ZF(JI,JJ,JK)                          &
          + PIMPL * CSTURB%XCTV*PLM(JI,JJ,JK)*PLEPS(JI,JJ,JK)*0.5 &
          * ZWORK6(JI,JJ,JK)                                                   &
          + PIMPL * ZDFDDTDZ(JI,JJ,JK) * ZWORK7(JI,JJ,JK)         &
          + PIMPL * ZDFDDRDZ(JI,JJ,JK) * ZWORK8(JI,JJ,JK)
        ENDDO
    ...
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ...
        *( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PRM(JI,JJ,IKB+2*D%NKL,1)             &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PRM(JI,JJ,IKB+D%NKL,1)             & 
        +ZCOEFF(JI,JJ,IKB)*PRM(JI,JJ,IKB,1))                &
        +PIMPL *                                                                              &
        ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PTHLP(JI,JJ,IKB+2*D%NKL)             &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PTHLP(JI,JJ,IKB+D%NKL)             &
        +ZCOEFF(JI,JJ,IKB)*PTHLP(JI,JJ,IKB))                &
        *( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PRP(JI,JJ,IKB+2*D%NKL)             &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PRP(JI,JJ,IKB+D%NKL)             & 
    ...
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZFLXZ(JI,JJ,IKB) =                                                            & 
        (CSTURB%XCHT1 * PPHI3(JI,JJ,IKB+D%NKL) + CSTURB%XCHT2 * PPSI3(JI,JJ,IKB+D%NKL))   &
        *( PEXPL *                                                                              &
        ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PTHLM(JI,JJ,IKB+2*D%NKL)             &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PTHLM(JI,JJ,IKB+D%NKL)             & 
        +ZCOEFF(JI,JJ,IKB)*PTHLM(JI,JJ,IKB))                &
...
!  NB PATHETA is -b in Chaboureau Bechtold 2002 which explains the + sign here
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PSIGS(JI,JJ,JK) = PSIGS(JI,JJ,JK) +     &
          2. * PATHETA(JI,JJ,JK) * PAMOIST(JI,JJ,JK) * ZFLXZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
...
IF (OHARAT) THEN
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK1(JI,JJ,JK) = PDR_DZ(JI,JJ,JK)**2
        ENDDO
      ENDDO
    ENDDO
  CALL MZF_PHY(D,ZWORK1,ZWORK2)
...
ELSE
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK1(JI,JJ,JK) = PPSI3(JI,JJ,JK)*PDR_DZ(JI,JJ,JK)**2
        ENDDO
      ENDDO
    ENDDO
  CALL MZF_PHY(D,ZWORK1,ZWORK2)
...
ENDIF
    ...
    DO JK=1,D%NKT 
      DO JJ=1,D%NJT 
        DO JI=1,D%NIT 
          ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFR2(JI,JJ,JK)
          ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFR2(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    END IF
    ...
    IF (KRR/=0) THEN
      ...
      DO JK=1,D%NKT 
        DO JJ=1,D%NJT 
          DO JI=1,D%NIT 
            ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFTH2(JI,JJ,JK) 
            ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFTH2(JI,JJ,JK)
          ENDDO
        ENDDO
      ENDDO
      END IF
      ...
      DO JK=1,D%NKT 
        DO JJ=1,D%NJT 
          DO JI=1,D%NIT 
            ZF(JI,JJ,JK) = ZF(JI,JJ,JK) + ZWORK1(JI,JJ,JK) * PFTHR(JI,JJ,JK) 
            ZDFDDRDZ(JI,JJ,JK) = ZDFDDRDZ(JI,JJ,JK) + ZWORK2(JI,JJ,JK) * PFTHR(JI,JJ,JK)
          ENDDO
        ENDDO
      ENDDO
      END IF
  ...
  IF (OHARAT) THEN
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK5(JI,JJ,JK) = ZWORK2(JI,JJ,JK) / PDZZ(JI,JJ,JK)
          ZWORK3(JI,JJ,JK) = 2.*PDR_DZ(JI,JJ,JK)* ZWORK5(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    CALL MZF_PHY(D,ZWORK3,ZWORK4)
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZFLXZ(JI,JJ,JK) = ZF(JI,JJ,JK)                   &
          + PIMPL * PLMF(JI,JJ,JK) *PLEPSF(JI,JJ,JK) &
          * ZWORK4(JI,JJ,JK) &
          + PIMPL * ZDFDDRDZ(JI,JJ,JK) * ZWORK6(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
   ELSE
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK1(JI,JJ,JK) = ZWKPHIPSI1(JI,JJ,JK)*ZWORK2(JI,JJ,JK) &
          / PDZZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK4(JI,JJ,JK) = ZWORK2(JI,JJ,JK) / PDZZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZFLXZ(JI,JJ,JK) = ZF(JI,JJ,JK)                             &
          + PIMPL * CSTURB%XCTV*PLM(JI,JJ,JK) *PLEPS(JI,JJ,JK) &
          * ZWORK3(JI,JJ,JK) &
          + PIMPL * ZDFDDRDZ(JI,JJ,JK) * ZWORK5(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO  
  ENDIF
    ...
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ...
        *( PEXPL *                                                  &
        ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PRM(JI,JJ,IKB+2*D%NKL,1)             &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PRM(JI,JJ,IKB+D%NKL,1)             & 
        +ZCOEFF(JI,JJ,IKB)*PRM(JI,JJ,IKB,1))**2         &
        +PIMPL *                                                  &
        ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PRP(JI,JJ,IKB+2*D%NKL)               &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PRP(JI,JJ,IKB+D%NKL)               &
        +ZCOEFF(JI,JJ,IKB)*PRP(JI,JJ,IKB))**2           &
        )
      ENDDO
    ENDDO
   ELSE
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZFLXZ(JI,JJ,IKB) = CSTURB%XCHV * PPSI3(JI,JJ,IKB+D%NKL) * PLM(JI,JJ,IKB)   &
        * PLEPS(JI,JJ,IKB)                                        &
        *( PEXPL *                                                  &
        ( ZCOEFF(JI,JJ,IKB+2*D%NKL)*PRM(JI,JJ,IKB+2*D%NKL,1)             &
        +ZCOEFF(JI,JJ,IKB+D%NKL)*PRM(JI,JJ,IKB+D%NKL,1)             & 
  ...
  ENDDO
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PSIGS(JI,JJ,JK) = PSIGS(JI,JJ,JK) + PAMOIST(JI,JJ,JK) **2 &
          * ZFLXZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
...
!
  ...
  DO JJ=IJB,IJE 
    DO JI=IIB,IIE 
      PSIGS(JI,JJ,D%NKA) = PSIGS(JI,JJ,IKB)
      PSIGS(JI,JJ,D%NKU) = PSIGS(JI,JJ,IKE)
    ENDDO
  ENDDO
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
#ifdef REPRO48
        PSIGS(JI,JJ,JK) =  MAX (PSIGS(JI,JJ,JK) , 0.)
        PSIGS(JI,JJ,JK) =  SQRT(PSIGS(JI,JJ,JK))
#else
        PSIGS(JI,JJ,JK) =  SQRT( MAX (PSIGS(JI,JJ,JK) , 1.E-12) )
#endif
      ENDDO
    ENDDO
  ENDDO
