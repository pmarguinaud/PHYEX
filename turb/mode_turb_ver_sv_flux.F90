...
IF (OHARAT) THEN
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZKEFF(JI,JJ,JK+IKB-(1) ) =  PLM(JI,JJ,JK) * SQRT(PTKEM(JI,JJ,JK)) & 
        + 50.*MFMOIST(JI,JJ,JK)
      ENDDO
    ENDDO
  ENDDO
...
! Preparation of the arguments for TRIDIAG 
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE   
          ZA(JI,JJ,JK+IKB-(1) ) = -PTSTEP * ZKEFF(JI,JJ,JK) * ZWORK1(JI,JJ,JK) &
          / PDZZ(JI,JJ,JK)**2
        ENDDO
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZA(JI,JJ,JK+IKB-(1) ) = -PTSTEP*ZCSV*PPSI_SV(JI,JJ,JK,JSV) *   &
          ZKEFF(JI,JJ,JK) * ZWORK1(JI,JJ,JK) / PDZZ(JI,JJ,JK)**2
        ENDDO
      ENDDO
    ENDDO
    ENDIF
...
! is taken into account in the vertical part
  IF (HTURBDIM=='3DIM') THEN
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        ZSOURCE(JI,JJ,IKB) = (PIMPL*PSFSVP(JI,JJ,JSV) + PEXPL*PSFSVM(JI,JJ,JSV)) / &
        PDZZ(JI,JJ,IKB) * PDIRCOSZW(JI,JJ)                    &
...
!  Compute the equivalent tendency for the JSV scalar variable
  DO JK=1,D%NKT 
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        PRSVS(JI,JJ,JK+IKB-(1) ,JSV)= PRSVS(JI,JJ,JK,JSV)+    &
        PRHODJ(JI,JJ,JK)*(ZRES(JI,JJ,JK)-PSVM(JI,JJ,JK,JSV))/PTSTEP
      ENDDO
    ENDDO
  ENDDO
!
  IF ( (OTURB_FLX .AND. TPFILE%LOPENED) .OR. OLES_CALL ) THEN
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZWORK1(JI,JJ,JK) = PLM(JI,JJ,JK)*SQRT(PTKEM(JI,JJ,JK))
          ZWORK2(JI,JJ,JK) = PIMPL*ZRES(JI,JJ,JK) + PEXPL*PSVM(JI,JJ,JK,JSV)
        ENDDO
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZFLXZ(JI,JJ,JK+IKB-(1) ) = -ZCSV * PPSI_SV(JI,JJ,JK,JSV) * ZWORK3(JI,JJ,JK) & 
          / PDZZ(JI,JJ,JK) * &
          ZWORK4(JI,JJ,JK)
        ENDDO
      ENDDO
    ...
    IF (HTURBDIM=='3DIM') THEN
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          ZFLXZ(JI,JJ,IKB) = (PIMPL*PSFSVP(JI,JJ,JSV) + PEXPL*PSFSVM(JI,JJ,JSV))  &
          * PDIRCOSZW(JI,JJ)  
        ENDDO
      ENDDO
    ELSE
    ...
    DO JK=IKTB+1,IKTE-1
      DO JJ=IJB,IJE 
        DO JI=IIB,IIE 
          PWSV(JI,JJ,JK,JSV)=0.5*(ZFLXZ(JI,JJ,JK)+ZFLXZ(JI,JJ,JK+D%NKL))
        ENDDO
      ENDDO
    END DO
    DO JJ=IJB,IJE 
      DO JI=IIB,IIE 
        PWSV(JI,JJ,IKB,JSV)=0.5*(ZFLXZ(JI,JJ,IKB)+ZFLXZ(JI,JJ,IKB+D%NKL))
        PWSV(JI,JJ,IKE,JSV)=PWSV(JI,JJ,IKE-D%NKL,JSV)
      ENDDO
    ENDDO
 END IF
  !
