...
! Preparation of the arguments for TRIDIAG 
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT   
      ZA(JI,JJ,JK) = -PTSTEP * ZKEFF(JI,JJ,JK) * ZWORK1(JI,JJ,JK) / PDZZ(JI,JJ,JK)**2
        ENDDO
 ENDDO
ENDDO
    ELSE
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
      ZA(JI,JJ,JK) = -PTSTEP*ZCSV*PPSI_SV(JI,JJ,JK,JSV) *   &
...
!
 ...
 DO JJ=1,D%NJT 
  DO JI=1,D%NIT 
    ZFLXZ(JI,JJ,JK) = -ZCSV * PPSI_SV(JI,JJ,JK,JSV) * ZWORK1(JI,JJ,JK) / PDZZ(JI,JJ,JK) * &
                  ZWORK2(JI,JJ,JK)
      ENDDO
 ENDDO
ENDDO
